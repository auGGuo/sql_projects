...
FROM '/Users/augustguo/Desktop/Postgres/practical_sql/us_counties_2010.csv'
...
# use arrary to store multiple values in one cell
...
# Identify New York state county has the highest percentage of the population 
# that identified as “American Indian/Alaska Native Alone.
SELECT geo_name,
	   round(((CAST (p0010005 AS numeric(8,1)) / p0010001) * 100), 2) 
	   AS "pct_native"
FROM us_counties_2010
...
FROM '/Users/augustguo/Desktop/Postgres/practical_sql/us_counties_2000.csv'
WITH (FORMAT CSV, HEADER);
...
-- Using appropriate joins and the NULL value, identify which counties 
...
-- Find median of the percent change in county population
SELECT
	percentile_cont(.5) 
	WITHIN GROUP (ORDER BY ((CAST(c2010.p0010001 AS NUMERIC(8,1)) 
							 - c2000.p0010001) / c2000.p0010001 * 100))							 
	AS "50th_percentile"
FROM us_counties_2010 c2010
